...
class EVIProtocol:
    ...
    async def request(self): 
        ...
        print(
            "EVI connector sending message with %d requests", len(message)
        )
    ...
    async def get_reply(self):
        ...
        while self.curr_updates > 0:
            ...
            for m in received:
                ...
                if message.HasField("vehicle"):
                    print("Got Message from EVI: %s", message)
                    for part in message.vehicle.commands: 
                        if part.HasField("register_vehicle_command"):
                            part.register_vehicle_command.is_ego_vehicle=False
                        if part.HasField("unregister_vehicle_command"):
                            part.unregister_vehicle_command.is_ego_vehicle=False
                        if part.HasField("update_vehicle_command"):
                            part.update_vehicle_command.is_ego_vehicle=False
        ...
        print(
            "EVI connector receving message with %d requests", len(received)
        )
